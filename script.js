...
const maxWinNum = document.getElementById("maxWinNum")
const minWinNum = document.getElementById("minWinNum")
const ApplyCustomChacesButton = document.getElementById("ApplyCustomChaces")
...
let mx = 7
let mn = 0
ApplyCustomChacesButton.onclick = () =>{
  mx0 = parseInt(maxWinNum.value)
  mn0 = parseInt(minWinNum.value)

  if (!isNaN(mx0) && !isNaN(mn0)){
    mx = parseInt(maxWinNum.value)
    mn = parseInt(minWinNum.value)
    alert(`Succesfully set chances to\nMin=${mn},Max=${mx}`)
  } 
  else if (isNaN(mx0)) alert("Max can't be null")
  else if (isNaN(mn0)) alert("Min can't be null")
}
...
spin.onclick = async () => {
  if (!spinning) {
    ...
    if (balance.value <= 0) {
      alert("No money to spin");
    }else {
      if (bet <= balance.value && bet >= 10) {
        ...
        alert("Bet needs to be min 10")
      } else {
        alert("Bet needs to be lower or equal");
      }
    }
  }else{
