...
async function main(balance, bet, delay, rounds,win_nums) {
  ...
  let dr1 = Math.floor(Math.random() * mx)
  let dr2 = Math.floor(Math.random() * mx);
  let dr3 = Math.floor(Math.random() * mx);
...
spin.onclick = async () => {
  if (!spinning) {
    ...
    const win_nums = {
      "r1":Math.floor(Math.random() * mx),
      "r2":Math.floor(Math.random() * mx),
      "r3":Math.floor(Math.random() * mx),
    } 
